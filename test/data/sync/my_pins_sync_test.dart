...
void main() {
  ...
  test(
    'downloads pins from the delta endpoint and writes them locally',
    () async {
  ...
  test(
    'respects local tombstones — does not re-insert pin user deleted',
    () async {
      final sync = _build('me');
  ...
  group('upload path', () {
    test(
      'queue optimization: DELETE supersedes earlier CREATE and UPDATE on same pin',
      () async {
        final sync = _build('me');
    ...
    test(
      'queue optimization: keeps only latest UPDATE when no DELETE present',
      () async {
        final sync = _build('me');
    ...
    test(
      'DELETE swallows "not found" exception as idempotent success',
      () async {
    ...
    test(
      'transient failure on CREATE increments retry count, op stays queued',
      () async {
        remote.insertError = Exception(
          'genuine network error not a known idempotent string',
        );
        final sync = _build('me');
    ...
    test(
      'just-deleted pin id suppresses re-insert in the subsequent download phase',
      () async {
