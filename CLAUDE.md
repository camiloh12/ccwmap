...
**Current Status:** v0.6.0 in production — pre-populate-pins Phases 0–5 complete (Phases 0–4 merged to master via PRs #25–#32; Phase 5 validated in **staging** with a PR open to merge `feature/pre-populate`; no prod import yet for any phase). Phase 4 (federal floor) on branch `feature/pre-populate`: GSA FRPP (xlsx, Census-geocoded, point-in-polygon coord-validated, Building-only, degenerate city/address labels recomposed) + USACE MIRTA military source modules, real cross-source dedup (shapely STRtree + rapidfuzz, case-insensitive, priority `user > … > gsa > hifld_* > osm`), multi-source pipeline. Applied to staging + idempotency-verified (gsa 4220 / courts 470 / military 77 = 4,767 for TX/FL/PA); owner-feedback fixes landed incl. one Flutter map change (in-place `setGeoJsonSource` to stop pan/zoom blink). No new migration (008 covers it). Phase 5 source modules are code-complete **with real-data pre-flight done** (on this branch): NCES public K-12 (`importer/importer/sources/nces.py`, SY 2024-25 EDGE geocode + CCD `SY_STATUS` join, ~16k TX/FL/PA), IPEDS colleges (`importer/importer/sources/ipeds.py`, HD2024, FL classified; TX/PA expected missing-cell drops at `apply_state_law`), and FAA commercial-service airports (`importer/importer/sources/faa.py`, CY2023 enplanements `S/L`=P/CS + 28-day NASR `APT_BASE.csv` coords, ~60 TX/FL/PA); shared archive/zip/xlsx fetch helpers in `importer/importer/sources/_archive.py`; live URLs pinned in `config.yaml`; fixtures are real captured rows; no new migration required. Phase 5 **applied to staging + idempotency-verified** (nces 15,432 / ipeds 290 FL-only / faa 60 = 15,782 new; staging total 20,550 across all sources, all system-owned; combined dry-run + apply + 2nd-apply INSERT-0; no new security/performance advisors; clustering eyeballed OK at Houston/Miami/Philadelphia). Next: merge `feature/pre-populate` to master. ODbL attribution UI, all-caps label title-casing, and the real prod import remain deferred. See `docs/superpowers/specs/2026-06-02-phase4-federal-floor-design.md` and `…/2026-05-10-pre-populate-pins-design.md` §8.
...
- ✅ Tests passing (run `flutter test` for the app; `cd importer && pytest` for the importer — 123 at Phase 5)
