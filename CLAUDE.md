...
**Current Status:** v0.6.0 in production — pre-populate-pins Phases 0–4 complete (Phase 4 validated in **staging**; not yet merged to master, no prod import). Phase 4 (federal floor) on branch `feature/pre-populate`: GSA FRPP (xlsx, Census-geocoded, point-in-polygon coord-validated, Building-only, degenerate city/address labels recomposed) + USACE MIRTA military source modules, real cross-source dedup (shapely STRtree + rapidfuzz, case-insensitive, priority `user > … > gsa > hifld_* > osm`), multi-source pipeline. Applied to staging + idempotency-verified (gsa 4220 / courts 470 / military 77 = 4,767 for TX/FL/PA); owner-feedback fixes landed incl. one Flutter map change (in-place `setGeoJsonSource` to stop pan/zoom blink). No new migration (008 covers it). Next: merge `feature/pre-populate` to master, then Phase 5 (schools + airports). ODbL attribution UI, all-caps label title-casing, and the real prod import remain deferred. See `docs/superpowers/specs/2026-06-02-phase4-federal-floor-design.md` and `…/2026-05-10-pre-populate-pins-design.md` §8.
