...
**Current Status:** v0.6.0 in production — pre-populate-pins Phases 0–6 complete and **staging-validated** (no prod import yet for any phase). Phases 0–5 are merged to master; a PR is open to merge **Phase 6** (`feature/pre-populate`) to master. Phase 4 (federal floor): GSA FRPP + USACE MIRTA military modules, real cross-source dedup (shapely STRtree + rapidfuzz, case-insensitive, priority `user > nces > ipeds > faa > gsa > hifld_* > osm`), multi-source pipeline; one Flutter map change (in-place `setGeoJsonSource` to stop pan/zoom blink). Phase 5 (schools+airports): NCES public K-12, IPEDS colleges (FL classified; TX/PA expected missing-cell drops at `apply_state_law`), FAA commercial-service airports. Phase 6 (OSM long-tail + ODbL + title-casing): Overpass `osm` source with per-state auto-scope driven by the state-law table (bars TX/FL only, PA generates no query) — bars pre-asserted **UNCERTAIN (yellow), NOT NO_GUN (red)** because OSM tags cannot confirm the TX 51% / FL "primarily-devoted" revenue test (owner decision 2026-06-16; `BAR_ALCOHOL/osm` cells in `data/state_laws/states.yaml`); a real **ODbL derived-database dump** (OSM-derived columns only — no legal classifications) gzip'd to the public `odbl-dumps` Storage bucket after each OSM apply (pipeline Phase D, idempotent bucket create, 90-day prune); all-caps label **title-casing** in `normalize` (acronym + state-code preserve-list). Overpass rejects the default httpx User-Agent with HTTP 406 — `osm.py` sends a descriptive UA. Staging total **23,825** system-owned pins (gsa 4221 / nces 15432 / osm 3275 / courts 470 / ipeds 290 / military 77 / faa 60); combined all-source apply + idempotency re-run INSERT-0 with stable counts, title-case wave settled (10 legit residual all-caps acronyms), no new security/performance advisors, ODbL dump downloads from its public URL. No new migration (008 covers it). Deferred: in-app ODbL attribution + a low-confidence "verify locally" caveat UI (all NO_GUN pins still render hard-red), and the real prod import. See `docs/superpowers/specs/2026-06-16-phase6-osm-longtail-design.md`, `…/2026-06-02-phase4-federal-floor-design.md`, and `…/2026-05-10-pre-populate-pins-design.md` §8. Low-zoom map rendering (ahead of Phase 7 prod rollout): refined the cluster bubbles to **uniform blue, small capped radius, abbreviated counts** (`4.5k`) — fixing the overlapping all-red blob the pre-populate data exposed (clusters no longer color by dominant status, which was nearly all `NO_GUN`) — on `feature/cluster-bubbles`. A native-heatmap and a crash-free circle-glow variant were explored and **parked** on `feature/heatmap-lowzoom` (the native `addHeatmapLayer` SIGSEGVs Android's TextureView render thread, matching unfixed maplibre-react-native#954). See `docs/dev/CLUSTER_RENDERING.md`.
**Target Platforms:** Android and iOS (production), Web (development/testing)
**Backend:** Supabase (PostgreSQL + Auth + Realtime)

### What's Implemented (Iterations 1-7 + v0.4.0 SP-1/2/3 + v0.6.0 pre-populate Phases 0–3)
...
- ✅ Tests passing (run `flutter test` for the app; `cd importer && pytest` for the importer — 152 at Phase 6)
