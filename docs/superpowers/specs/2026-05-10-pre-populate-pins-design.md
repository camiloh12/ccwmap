...
| 3 | **State-law table seeding** **(DONE 2026-05-31)** | Research + write the cells with a real categorical statutory prohibition for TX + FL + PA + federal-uniform US. | ✅ DONE — 12 web-verified, user-approved `NO_GUN` cells (3 US federal-uniform + TX 3 + FL 4 + PA 2), all dated `2026-05-31` with validated citations; the 12 intentional omissions documented in `docs/importer/OMISSIONS.md`; structural tests lock resolution, the campus-carry guard, omissions, and the source-filter drop-edge. | 1 week (mostly legal research; can parallel with other work) |
