...
- **IPEDS colleges.** Single IPEDS HD ("Directory information") CSV joined on `UNITID` (`INSTNM`, `STABBR`, `LATITUDE`, `LONGITUD`, `CYACTIVE`). Category `COLLEGE_UNIVERSITY`. Only FL has a `COLLEGE_UNIVERSITY` state-law cell; TX and PA candidates are emitted but drop at `apply_state_law` — these appear as "missing cells" in dry-run output. See `docs/importer/OMISSIONS.md` for the researched rationale. **Native coordinates** — no geocoding.
